...
class Facebook extends Component {
  ...
  urlPost = () => {
        fetch('http://localhost:3000/api/post/facebook', {headers: new Headers({
  ...
  Node = async () => {
    this.setState({data: [],statedata: null,passwordSate: null})
    await fetch('/api/get/facebook')
      .then(res => res.json())
      .then(passwords => {
  ...
  password(value, index) {
    this.setState({passwordSate: value})
}
